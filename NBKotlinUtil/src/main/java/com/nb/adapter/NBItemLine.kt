...
class NBItemLine : RecyclerView.ItemDecoration() {
    ...
    override fun getItemOffsets(outRect: Rect, view: View, parent: RecyclerView, state: RecyclerView.State) {
        when (orientation) {
            DIVIDER_VERTICAL -> outRect.set(0, 0, 0, (verticalSize * scaleSize))
            DIVIDER_HORIZONTAL -> outRect.set(0, 0, (horizontalSize * scaleSize), 0)
            DIVIDER_BOTH -> outRect.set(0, 0, (horizontalSize * scaleSize), (verticalSize * scaleSize))
            else
            -> {
            }
